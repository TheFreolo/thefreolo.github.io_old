...
La Visualizzazione dei dati può permettere di individuare trend, connessioni e carpire informazioni dai dati che in forma tabulare non sono ovvie.
...
Si noti che il pacchetto *car* contiene una funzione chiamata `Boxplot()`, con la maiuscola, che vicino ai potenziali outliers indica l'indice dell'elemento.
...
Il comando giusto specifica quale separatore viene considerato: in questo caso lo spazio, che è diverso da quello di default di `read.csv`
.
```{r}
